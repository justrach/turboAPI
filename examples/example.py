...
from starlette.requests import Request # Added for simplified endpoint
...
def create_item(item: Item, db=Depends(get_db)):
    ...
    - **item**: The item data
    """
    # Convert satya model to dict using vars() instead of dict()
    item_dict = vars(item)
...
def update_item(item_id: int, item: Item, db=Depends(get_db)):
    ...
    # Convert satya model to dict using vars()
    item_dict = vars(item)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"App has __call__: {hasattr(app, '__call__')}")
