...
def start_server(cmd: list, port: int, env: dict = None) -> subprocess.Popen:
    ...
    proc = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=full_env)
...
def run_benchmarks():
    ...
    print("=" * 70)
    print(
        f"Duration: {BENCHMARK_DURATION}s | Threads: {BENCHMARK_THREADS} | Connections: {BENCHMARK_CONNECTIONS}"
    )
    print("=" * 70)
    ...
    try:
        ...
        for name, path, method, body in benchmarks:
            ...
            result = run_wrk(
                url, BENCHMARK_DURATION, BENCHMARK_THREADS, BENCHMARK_CONNECTIONS, method, body
            )
