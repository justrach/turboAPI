...
/// True when the build was configured with `-Diouring=true` AND the
/// target is Linux. The Stream type in stream.zig switches to a
/// per-connection io_uring transport when this is true.
pub const has_iouring = build_config.iouring and @import("builtin").os.tag == .linux;
