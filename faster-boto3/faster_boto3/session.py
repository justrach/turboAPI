...
def _native_mode_enabled(service_name: str) -> bool:
...
class Session:
    ...
    def client(
        ...
        if service_name != "s3" or not _native_mode_enabled(service_name):
            return fallback

        return NativeS3Client.from_botocore_client(fallback)
