...
def start_server(script_path, port, server_name):
    ...
    if "fastapi" in script_path:
        ...
        process = subprocess.Popen(
            [sys.executable, script_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
    else:
...
def create_performance_graphs(results, output_dir="benchmark_graphs"):
    ...
    try:
        ...
        fig.suptitle(
            "🚀 TurboAPI vs FastAPI Performance Comparison", fontsize=20, fontweight="bold"
        )
        ...
        bars1 = ax1.bar(
            x - width / 2, turbo_rps, width, label="TurboAPI", color="#FF6B6B", alpha=0.8
        )
        bars2 = ax1.bar(
            x + width / 2, fastapi_rps, width, label="FastAPI", color="#4ECDC4", alpha=0.8
        )
        ...
        bars3 = ax3.bar(
            x - width / 2, turbo_latency, width, label="TurboAPI", color="#FF6B6B", alpha=0.8
        )
        bars4 = ax3.bar(
            x + width / 2, fastapi_latency, width, label="FastAPI", color="#4ECDC4", alpha=0.8
        )
...
def main():
    ...
    turbo_process = start_server(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "test.py"), 8080, "TurboAPI"
    )
    if turbo_process:
        results["TurboAPI"] = {}
    ...
    fastapi_process = start_server(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "fastapi_equivalent.py"),
        8081,
        "FastAPI",
    )
    if fastapi_process:
    ...
    print("\n🎯 Benchmark completed!")
    print("📝 Note: TurboAPI is running on Python 3.13+ free-threading")
    print("📝 Note: FastAPI is running on standard Python with uvicorn")
    print("\n📁 Output files:")
    ...
    if "TurboAPI" in results and "FastAPI" in results:
        ...
        total_turbo_rps = sum(
            results["TurboAPI"][b].get("rps", 0)
            for b in ["Light Load", "Medium Load", "Heavy Load"]
            if b in results["TurboAPI"]
        )
        total_fastapi_rps = sum(
            results["FastAPI"][b].get("rps", 0)
            for b in ["Light Load", "Medium Load", "Heavy Load"]
            if b in results["FastAPI"]
        )
        ...
        if total_fastapi_rps > 0:
            ...
            print(
                f"⚡ Total RPS - TurboAPI: {total_turbo_rps:.0f} | FastAPI: {total_fastapi_rps:.0f}"
            )
            ...
            if best_scenario:
                print(
                    f"🎯 Best Performance: {best_scenario} with {best_improvement:.1f}% improvement"
                )
