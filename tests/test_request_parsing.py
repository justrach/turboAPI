...
def test_path_parameters():
    ...
    def get_product(category: str, product_id: str, details: str = "basic"):
        return {"category": category, "product_id": product_id, "details": details}
...
def test_headers():
    ...
    response = requests.get(
        "http://127.0.0.1:9102/auth", headers={"Authorization": "Bearer token123"}
    )
    ...
    response = requests.get(
        ...
        headers={"User-Agent": "TurboAPI-Test/1.0", "Accept": "application/json"},
    )
    ...
    response = requests.get(
        "http://127.0.0.1:9102/api-key", headers={"X-API-Key": "secret-key-123"}
    )
...
def test_combined_parameters():
    ...
    # Verify headers
    assert result["headers"]["has_auth"] is True
