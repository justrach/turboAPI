...
def run_benchmark_suite(framework, port, script_name):
    ...
    if framework == "TurboAPI":
        process = subprocess.Popen(
            [sys.executable, "tests/test.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    else:  # FastAPI
        process = subprocess.Popen(
    ...
    # Test endpoints
    endpoints = ["/", "/benchmark/simple", "/benchmark/medium", "/benchmark/json"]

    results = {"framework": framework, "port": port, "endpoints": {}}
    ...
    for endpoint in endpoints:
        ...
        if sequential_stats:
            print(
                f"  Sequential - Mean: {sequential_stats['mean'] * 1000:.2f}ms, P95: {sequential_stats['p95'] * 1000:.2f}ms"
            )
        if concurrent_stats:
            print(
                f"  Concurrent - Mean: {concurrent_stats['mean'] * 1000:.2f}ms, P95: {concurrent_stats['p95'] * 1000:.2f}ms"
            )
...
def compare_frameworks():
    ...
    for endpoint in turbo_results["endpoints"].keys():
        ...
        if turbo_ep["sequential"] and fastapi_ep["sequential"]:
            ...
            print("Sequential Response Time:")
            print(f"  TurboAPI: {turbo_seq:.2f}ms")
            print(f"  FastAPI:  {fastapi_seq:.2f}ms")
            print(
                f"  TurboAPI is {improvement:.1f}x faster"
                if improvement > 1
                else f"  FastAPI is {1 / improvement:.1f}x faster"
            )
        ...
        if turbo_ep["concurrent"] and fastapi_ep["concurrent"]:
            ...
            print("Concurrent Response Time:")
        ...
        if turbo_rps > 0 and fastapi_rps > 0:
            ...
            print("  TurboAPI significantly outperforms FastAPI")
    ...
    for endpoint in turbo_results["endpoints"].keys():
        ...
        if turbo_ep["sequential"] and fastapi_ep["sequential"]:
            seq_improvements.append(
                fastapi_ep["sequential"]["mean"] / turbo_ep["sequential"]["mean"]
            )

        if turbo_ep["concurrent"] and fastapi_ep["concurrent"]:
            conc_improvements.append(
                fastapi_ep["concurrent"]["mean"] / turbo_ep["concurrent"]["mean"]
            )

        if turbo_ep["max_sustainable_rps"] > 0 and fastapi_ep["max_sustainable_rps"] > 0:
            rps_improvements.append(
                turbo_ep["max_sustainable_rps"] / fastapi_ep["max_sustainable_rps"]
            )
    ...
    with open("benchmark_results.json", "w") as f:
        json.dump(
            {"turboapi": turbo_results, "fastapi": fastapi_results, "timestamp": time.time()},
            f,
            indent=2,
        )
