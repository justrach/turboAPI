...
def start_turboapi():
    ...
    process = subprocess.Popen(
        [sys.executable, "tests/test.py"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
    time.sleep(3)
...
def kill_port(port):
    ...
    try:
        subprocess.run(
            f"lsof -ti:{port} | xargs kill -9 2>/dev/null", shell=True, capture_output=True
        )
        time.sleep(1)
    except Exception:
        pass
...
def generate_visualization(results):
    ...
    try:
        ...
        fig.suptitle("TurboAPI vs FastAPI Performance Comparison", fontsize=16, fontweight="bold")
        ...
        for idx, endpoint in enumerate(endpoints):
            ...
            turbo_rps = [results["turboapi"][config][endpoint].get("rps", 0) for config in configs]
            fast_rps = [results["fastapi"][config][endpoint].get("rps", 0) for config in configs]

            bars1 = ax.bar(
                x - width / 2, turbo_rps, width, label="TurboAPI", color=colors_turbo, alpha=0.8
            )
            bars2 = ax.bar(
                x + width / 2, fast_rps, width, label="FastAPI", color=colors_fast, alpha=0.8
            )
            ...
            for i, (t, f) in enumerate(zip(turbo_rps, fast_rps, strict=False)):
                if f > 0:
                    speedup = t / f
                    ax.text(
                        i,
...
def main():
    ...
    for config in configs:
        ...
        for endpoint, name in endpoints:
            ...
            metrics, _ = run_wrk(
                url, config["threads"], config["connections"], config["duration"], wrk_path
            )
            results["turboapi"][config["name"]][name] = metrics

            if "rps" in metrics:
                print(
                    f"  {name}: {metrics['rps']:,.0f} req/s, "
