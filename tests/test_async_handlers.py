...
def test_sync_handler():
    ...
    # Test POST
    response = requests.post("http://127.0.0.1:9700/sync", json={"test": "data"})
...
def test_async_handler_basic():
    ...
    # Test POST
    response = requests.post("http://127.0.0.1:9701/async", json={"test": "async_data"})
...
def test_async_with_query_params():
    ...
    async def async_search(q: str, limit: str = "10"):
        await asyncio.sleep(0.01)
        return {"query": q, "limit": limit, "results": ["result1", "result2"], "async": True}
...
def test_async_with_headers():
    ...
    response = requests.get(
        "http://127.0.0.1:9703/auth", headers={"Authorization": "Bearer token123"}
    )
...
def test_async_with_large_payload():
    ...
    async def async_process(items: list):
        ...
        # Simulate processing
        count = len(items)
        return {"processed": count, "first_item": items[0] if items else None, "async": True}
    ...
    response = requests.post("http://127.0.0.1:9704/process", json=large_payload)
